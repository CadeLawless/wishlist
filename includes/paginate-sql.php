...
if($type == "wisher"){
    ...
    $findPriceTotal = $db->select("SELECT SUM(items.price * items.quantity) AS total_price FROM items LEFT JOIN wishlists ON items.wishlist_id = wishlists.id WHERE items.wishlist_id = ? AND wishlists.username = ?", [$wishlist_id, $username]);
